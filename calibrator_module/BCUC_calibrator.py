...
class BCUC_Calibrator:
    def __init__(
        ...
        # Logs
        self.interval_widths = []   # per-step (vector) 2*sigma
        self.Q_history = []
        self.q_history = []
        self.meu_history = []
        ...
        for obs, action in zip(observations, actions):
            ...
            if not np.isnan(obs).any():
                ...
                if self.mode == "quantile_coupled":
                    ...
                    E_bar = float(np.mean(self.conformal_p_control.E)) if len(self.conformal_p_control.E) else 0.0
                    if len(self.conformal_p_control.S):
                        try:
                            self.S_max = max(self.S_max, float(np.max(self.conformal_p_control.S)))
                        except Exception:
