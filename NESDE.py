...
#             V_batch = V.repeat(et.shape[0], 1, 1)           # shape [B, n, n]
#             V_inv_batch = torch.linalg.inv(V).repeat(et.shape[0], 1, 1)  # shape [B, n, n]

#             Phi.append(torch.bmm(V_batch, torch.nan_to_num(et).view(-1, n, n)))
...
#             Phi_inv.append(torch.bmm(et_inv.view(-1, n, n), V_inv_batch))
...
def Phi_(times, lambdas, V, n, time_par=False):
    ...
    if time_par:
        times = times.reshape(-1, 1)  # shape (B, 1)
        exp_diag = np.exp(times @ lambdas.reshape(1, -1))  # shape (B, n)
        et = np.array([np.diag(e) for e in exp_diag])      # shape (B, n, n)
        Phi = np.matmul(V[None, :, :], et)                 # shape (B, n, n)

        exp_diag_inv = np.exp(times @ (-lambdas).reshape(1, -1))
        et_inv = np.array([np.diag(e) for e in exp_diag_inv])  # shape (B, n, n)
        V_inv = np.linalg.inv(V)
        Phi_inv = np.matmul(et_inv, V_inv[None, :, :])     # shape (B, n, n)
        ...
        for t in times:
            t = t.reshape(-1, 1)  # shape (B, 1)
            exp_diag = np.exp(t @ lambdas.reshape(1, -1))  # shape (B, n)
            et = np.array([np.diag(e) for e in exp_diag])  # shape (B, n, n)

            B = et.shape[0]
            V_batch = np.repeat(V[None, :, :], B, axis=0)       # shape (B, n, n)
            V_inv_batch = np.repeat(np.linalg.inv(V)[None, :, :], B, axis=0)  # shape (B, n, n)

            Phi.append(np.matmul(V_batch, et))

            exp_diag_inv = np.exp(t @ (-lambdas).reshape(1, -1))
            et_inv = np.array([np.diag(e) for e in exp_diag_inv])  # shape (B, n, n)
            Phi_inv.append(np.matmul(et_inv, V_inv_batch))
...
def analytic_int(exps, t, complex, var, t0=None, exps_conj=None, ext_prod=None):
    ...
    if complex:
        if var:
            ...
            cos_1, sin_1 = integ_exp_trig(-expon, trig1, t, t0)
            cos_2, sin_2 = integ_exp_trig(-expon, trig2, t, t0)
            ...
            ind11 = np.tile(var_ind11, (n, n))
            ind12 = np.tile(var_ind12, (n, n))
            ind21 = np.tile(var_ind21, (n, n))
            ind22 = np.tile(var_ind22, (n, n))
            ind_cor = np.ones((n, n), dtype=bool)

            dot_11 = ext_prod[..., ind11].reshape(-1, n, n)
            dot_12 = ext_prod[..., ind12].reshape(-1, n, n)
            dot_21 = ext_prod[..., ind21].reshape(-1, n, n)
            dot_22 = ext_prod[..., ind22].reshape(-1, n, n)
            ...
            ind11 = np.block([[var_ind11] * n])
            ind12 = np.block([[var_ind12] * n])
            ind21 = np.block([[var_ind21] * n])
            ind22 = np.block([[var_ind22] * n])
            ind_cor = np.ones(n, dtype=bool)

            cos_int, sin_int = integ_exp_trig(-expon, trig, t, t0)

            mul_int = np.zeros_like(ext_prod).reshape(-1, 2 * n, 1).repeat(2 * n, axis=2)

            mul_int[..., ind11] = cos_int[..., ind_cor]
            mul_int[..., ind12] = -sin_int[..., ind_cor]
            mul_int[..., ind21] = sin_int[..., ind_cor]
            mul_int[..., ind22] = cos_int[..., ind_cor]

            res = np.matmul(mul_int, ext_prod)
            return res

    else:
        min_val = min_val_param
        abs_exps = np.clip(np.abs(exps), a_min=min_val, a_max=None)
        signs = np.sign(exps)

        if t0 is None:
            integ = (1.0 / abs_exps) * signs * (np.exp(exps * t) - 1.0)
        else:
            integ = (1.0 / abs_exps) * signs * (np.exp(exps * (t - t0)) - 1.0)

        return np.nan_to_num(integ)
...
class ESDE():
    ...
    #     # X0 = to_tensor(X0, torch.float64)
    #     # # X0 = X0.type(torch.float64)
    #     # X0_var = to_tensor(X0_var, torch.float64) 
    #     # # X0_var = X0_var.type(torch.float64)
    #     # U = to_tensor(U, torch.float64) 
    #     # # U = U.type(torch.float64)
    #     # times = to_tensor(times, torch.float64) 
    #     # # times = times.type(torch.float64)
    #     # Q = to_tensor(params['Q'], torch.float64) 
    #     Q = params['Q']
    ...
    #     else:
    #         lambdas =to_tensor(params['lambda'], torch.float32)
    #         lambdas = lambdas.unsqueeze(0)
    #         V = to_tensor(params['V'], torch.float32)
    #         V = V.unsqueeze(0)
    #         # compute initial conditions:
    #         Phi0, Phi0_inv = self.Phi(torch.zeros_like(times, device=self.device), lambdas, V, self.n)
    #         Phi0_inv = Phi0_inv[0]
    #         Phi0 = Phi0[0]
    #         X0_var = X0_var.unsqueeze(0)
    #         X0_expanded = X0.expand(times.shape[0], -1, -1)  # [T, n, 1]
    #         X0_var_expanded = X0_var.expand(times.shape[0], -1, -1)  # [T, n, n]
    #         init_cond = torch.bmm(Phi0_inv, X0_expanded)
    #         init_var = torch.bmm(torch.bmm(Phi0_inv, X0_var_expanded), Phi0_inv.transpose(-2, -1))
    #         # compute the fundemental matrix at the given time:
    #         Phi, _ = self.Phi(times, lambdas, V, self.n)
    #         Phi = Phi[0].view(-1, self.n, self.n)
    #         # Homogeneous solution:
    #         Hom = torch.bmm(Phi, init_cond)
    #         Hom_var = torch.bmm(torch.bmm(Phi, init_var), Phi.transpose(-2, -1))
    #         eye = torch.eye(self.n, device=Hom_var.device).view(1, self.n, self.n).repeat(Hom_var.shape[0], 1, 1)
    #         Hom_var = Hom_var * (
    #             torch.diag_embed(torch.diagonal(torch.sign(Hom_var), dim1=-2, dim2=-1)) + torch.ones_like(Hom_var, device=Hom_var.device) - eye
    #         )
    ...
    #         Q = Q.unsqueeze(0)
    #         Q_expanded = Q.expand(times.shape[0],-1,-1)
    #         Part_var = sig_int * torch.bmm(Phi0_inv, torch.bmm(Q_expanded, Phi0_inv.transpose(-1,-2)))
    #         Part_var = torch.bmm(Phi0, torch.bmm(Part_var, Phi0.transpose(-1, -2)))
    #         Part_var = Part_var * (torch.diag_embed(torch.diagonal(torch.sign(Part_var),dim1=-2,dim2=-1)) + torch.ones_like(Part_var, device=self.device)
    #                                 - eye)
    ...
    def predict_an(self, X0, X0_var, U, times, params):
        ...
        Q = params['Q']
        X0 = X0.reshape(-1, self.n, 1)

        if self.complex:
            lambdas = params['lambda'].astype(np.complex128)
            V = params['V'].astype(np.complex128)
            Phi0, Phi0_inv = self.Phi(np.zeros_like(times), lambdas, V, self.n)
            Phi0_inv = Phi0_inv[0]
            init_cond = Phi0_inv @ X0
            init_var = Phi0_inv @ X0_var @ Phi0_inv.swapaxes(-2, -1)
            ...
            diag_matrix = np.eye(self.n)[None, :, :] * diag_sign[:, :, None]
            Hom_var = Hom_var * (diag_matrix + 1 - np.eye(self.n)[None, :, :])

            ext_prod_lambdas = Phi0_inv @ U.reshape(-1, self.n, 1)
            lambda_int = analytic_int(lambdas, times.reshape(-1, 1), self.complex, False, ext_prod=ext_prod_lambdas).reshape(-1, self.n, 1)
            Part = np.matmul(Phi, lambda_int)

            ext_prod_siggs = Phi0_inv @ Q @ Phi0_inv.swapaxes(-1, -2)
            lambdas_siggs = lambdas[:, None, :] + lambdas[:, :, None]
            lambdas_siggs_conj = lambdas[:, None, :] - lambdas[:, :, None]
            sig_int = analytic_int(lambdas_siggs, times.reshape(-1, 1, 1), self.complex, True,
                                exps_conj=lambdas_siggs_conj, ext_prod=ext_prod_siggs)
            Part_var = Phi @ sig_int @ Phi.swapaxes(-1, -2)

        else:
            lambdas = np.expand_dims(params['lambda'].astype(np.float32), axis=0)
            V = np.expand_dims(params['V'].astype(np.float32), axis=0)
            Phi0, Phi0_inv = self.Phi(np.zeros_like(times), lambdas, V, self.n)
            Phi0_inv = Phi0_inv[0]
            Phi0 = Phi0[0]

            X0_var = np.expand_dims(X0_var, axis=0)
            X0_expanded = np.tile(X0, (times.shape[0], 1, 1))
            X0_var_expanded = np.tile(X0_var, (times.shape[0], 1, 1))

            init_cond = Phi0_inv @ X0_expanded
            init_var = Phi0_inv @ X0_var_expanded @ Phi0_inv.swapaxes(-2, -1)
            ...
            diag_sign = np.sign(np.diagonal(Hom_var, axis1=-2, axis2=-1))
            diag_matrix = np.eye(self.n)[None, :, :] * diag_sign[:, :, None]
            Hom_var = Hom_var * (diag_matrix + 1 - eye)

            lambda_int = analytic_int(lambdas, times.reshape(-1, 1), self.complex, False).reshape(-1, self.n, 1)
            ext_prod = Phi0_inv @ U.reshape(-1, self.n, 1)
            Part = Phi0 @ (lambda_int * ext_prod)

            lambdas_siggs = lambdas[:, None, :] + lambdas[:, :, None]
            sig_int = analytic_int(lambdas_siggs, times.reshape(-1, 1, 1), self.complex, True)

            Q = np.expand_dims(Q, axis=0)
            Q_expanded = np.tile(Q, (times.shape[0], 1, 1))

            Part_var = sig_int * (Phi0_inv @ Q_expanded @ Phi0_inv.swapaxes(-1, -2))
            Part_var = Phi0 @ Part_var @ Phi0.swapaxes(-1, -2)

            diag_sign = np.sign(np.diagonal(Part_var, axis1=-2, axis2=-1))
            diag_matrix = np.eye(self.n)[None, :, :] * diag_sign[:, :, None]
            Part_var = Part_var * (diag_matrix + 1 - eye)

        Sol = Hom.reshape(-1, self.n) + Part.reshape(-1, self.n)
        Sol_var = (Hom_var + Part_var).reshape(-1, self.n, self.n)

        # print(Part_var[-1])
        return Sol.astype(np.float64), Sol_var.astype(np.float64)


    # def conditional_dist(self, dist_mu, dist_sigma, smp_mask, smp, R):
    ...
    #     # dist_mu = to_tensor(dist_mu, torch.float64) 
    #     # dist_sigma = to_tensor(dist_sigma, torch.float64)
    #     # smp = to_tensor(smp, torch.float64)
    #     # smp_mask = to_tensor(smp_mask, torch.bool)
    ...
    #     R_obs = R[smp_mask][:, smp_mask]            # shape: [k, k]
    #     sigma22 = dist_sigma[..., smp_mask, :][..., :, smp_mask] + R_obs
        
    #     mask = torch.linalg.det(sigma22) == 0
    #     mu_mask = mask.view(-1, 1).repeat(1, mu1.shape[1])
    #     if sigma11.dim() == 2:
    #         sigma11 = sigma11.unsqueeze(0) # make sure sigma11 is 3D    
    #         sigma12 = sigma12.unsqueeze(0) # make sure sigma12 is 3D
    #         sigma21 = sigma21.unsqueeze(0) # make sure sigma21 is 3D
    #         sigma22 = sigma22.unsqueeze(0)
    ...
    #     cond_dist_mu = torch.zeros_like(dist_mu, device=self.device)
    #     cond_dist_sigma = torch.zeros_like(dist_sigma, device=self.device)

    #     cond_dist_mu[...,smp_mask] = smp.view(-1)
    ...
    def conditional_dist(self, dist_mu, dist_sigma, smp_mask, smp, R):
        ...
        smp_mask = np.array(smp_mask, dtype=bool)
        not_smp_mask = ~smp_mask
        ...
        sigma11 = dist_sigma[np.ix_(not_smp_mask, not_smp_mask)]
        sigma12 = dist_sigma[np.ix_(not_smp_mask, smp_mask)]
        sigma21 = dist_sigma[np.ix_(smp_mask, not_smp_mask)]
        sigma22 = dist_sigma[np.ix_(smp_mask, smp_mask)] + R[np.ix_(smp_mask, smp_mask)]

        # Handle singular sigma22
        if np.linalg.det(sigma22) == 0:
            mu_cond = mu1
            sigma_cond = sigma11
        else:
            sigma22_inv = np.linalg.inv(sigma22)
            delta = smp[smp_mask].reshape(-1, 1) - mu2.reshape(-1, 1)
            mu_cond = mu1 + sigma12 @ sigma22_inv @ delta
            sigma_cond = sigma11 - sigma12 @ sigma22_inv @ sigma21

        # Assemble the full conditional mean
        cond_dist_mu = np.zeros_like(dist_mu)
        cond_dist_mu[smp_mask] = smp[smp_mask]
        cond_dist_mu[not_smp_mask] = mu_cond.flatten()

        # Assemble the full conditional covariance
        cond_dist_sigma = np.zeros_like(dist_sigma)
        not_id_sig = np.outer(not_smp_mask.astype(float), not_smp_mask.astype(float)).astype(bool)
        cond_dist_sigma[not_id_sig] = sigma_cond.flatten()

        # Stabilize the covariance (equivalent to: diag_embed(sign) + 1 - I)
        sign_diag = np.sign(np.diagonal(cond_dist_sigma))
        sign_matrix = np.diag(sign_diag)
        eye = np.eye(cond_dist_sigma.shape[-1])
        cond_dist_sigma *= sign_matrix + (1 - eye)
